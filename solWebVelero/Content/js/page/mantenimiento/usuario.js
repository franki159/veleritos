...
function ListaLocales() {
    $.ajax({
        type: "POST",
        url: "permiso.aspx/ListarLocalWM",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: true,
        beforeSend: function () {
        ...
        success: function (data, status) {
            ...
            for (var i = 0; i < data.d.Resultado.length; i++) {
                contLocales += "<label class='checkbox-inline'>";
                contLocales += "<input type='checkbox' name='rad_local' value='" + data.d.Resultado[i].ID_LOCAL + "' />" + data.d.Resultado[i].DESCRIPCION;
                contLocales += "</label>";
            }

            $("#divlocales").html(contLocales);
        },
